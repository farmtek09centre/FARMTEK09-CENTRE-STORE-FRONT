...
function makeOrderId() { const n = new Date(); const date = `${n.getFullYear()}${String(n.getMonth()+1).padStart(2,"0")}${String(n.getDate()).padStart(2,"0")}`; return `FT09-${date}-${crypto.randomBytes(3).toString("hex").toUpperCase()}`; }
function makePublicToken() { return crypto.randomBytes(12).toString("hex"); }
function normalizeText(value, max = 200) { return String(value ?? "").trim().replace(/\s+/g, " ").slice(0, max); }
async function getAccessToken() { const auth = Buffer.from(`${process.env.MPESA_CONSUMER_KEY}:${process.env.MPESA_CONSUMER_SECRET}`).toString("base64"); const r = await fetch(`${MPESA_BASE_URL}/oauth/v1/generate?grant_type=client_credentials`, { headers: { Authorization: `Basic ${auth}`, Accept: "application/json" } }); const body = await r.text(); let data = {}; try { data = JSON.parse(body); } catch {} if (!r.ok || !data.access_token) throw new Error(data.errorMessage || data.error || `OAuth failed (HTTP ${r.status}).`); return data.access_token; }
...
async function validateAndPriceItems(items) {
  ...
  for (const raw of items) {
    ...
    normalized.push({ productId: product.id, name: product.name, quantity, unitPrice: Number(product.price), lineTotal: Number(product.price) * quantity });
  }
  return { normalized, subtotal: normalized.reduce((sum, item) => sum + item.lineTotal, 0) };
}
function publicOrder(order) { return { id: order.id, createdAt: order.createdAt, customerName: order.customerName, deliveryMethod: order.deliveryMethod, deliveryZone: order.deliveryZone, items: order.items, subtotal: order.subtotal, deliveryFee: order.deliveryFee, total: order.total, status: order.status, paymentMethod: order.paymentMethod, paymentStatus: order.paymentStatus, mpesaReceipt: order.mpesaReceipt || null }; }
async function applySuccessfulPayment(orderId, transaction) {
  ...
  order.paymentStatus = "paid"; order.status = "paid"; order.paidAt = new Date().toISOString(); order.paymentReference = transaction.receipt || transaction.checkoutRequestId || null; order.mpesaReceipt = transaction.receipt || null;
...
app.post("/api/orders", async (req, res) => {
  try {
    ...
    const order = { id: makeOrderId(), publicToken: makePublicToken(), createdAt: now, updatedAt: now, customerName, phone, email, deliveryMethod, deliveryZone, address, notes, items: normalized, subtotal, deliveryFee, total, paymentMethod, paymentStatus: "unpaid", status: "new", stockDeducted: false, timeline: [{ status: "new", at: now }] };
    const orders = await readOrders(); orders.unshift(order); await writeOrders(orders);
    res.status(201).json({ ...publicOrder(order), trackingToken: order.publicToken, paymentReady: deliveryFee !== null });
  } catch (error) { res.status(400).json({ message: error.message || "Unable to create the order." }); }
});
app.get("/api/orders/:id", async (req, res) => { try { const orders = await readOrders(); const order = orders.find((o) => o.id === req.params.id && o.publicToken === String(req.query.token || "")); if (!order) return res.status(404).json({ message: "Order not found." }); res.json({ ...publicOrder(order), trackingToken: order.publicToken }); } catch { res.status(500).json({ message: "Order tracking is unavailable." }); } });
...
app.get("/api/admin/customers", adminAuth, async (_req, res) => { const orders = await readOrders(); const grouped = new Map(); for (const order of orders) { const key = order.phone || order.email || order.customerName; if (!grouped.has(key)) grouped.set(key, { name: order.customerName, phone: order.phone, email: order.email, orders: 0, spend: 0, lastOrderAt: order.createdAt }); const c = grouped.get(key); c.orders += 1; if (order.paymentStatus === "paid") c.spend += Number(order.total || 0); if (order.createdAt > c.lastOrderAt) c.lastOrderAt = order.createdAt; } res.json({ customers: [...grouped.values()].sort((a,b)=>b.lastOrderAt.localeCompare(a.lastOrderAt)) }); });
app.get("/api/health", (_req, res) => res.json({ ok: true, service: "farmtek09-centre", environment: isProduction ? "production" : "sandbox" }));

app.post("/api/mpesa/stk-push", async (req, res) => { try { const orderId = normalizeText(req.body?.orderId, 60); const orders = orderId ? await readOrders() : []; const order = orderId ? orders.find((o) => o.id === orderId) : null; const phone = cleanPhone(req.body?.phone || order?.phone); const amount = normalizeAmount(req.body?.amount || order?.total); const accountReference = String(order?.id || req.body?.accountReference || "FARMTEK09").replace(/[^A-Za-z0-9]/g, "").slice(0, 12) || "FARMTEK09"; if (!phone) return res.status(400).json({ ok:false, message:"Enter a valid Kenyan M-PESA number." }); if (!amount) return res.status(400).json({ ok:false, message:"Enter a valid whole-number amount." }); if (order && (!order.total || amount !== Number(order.total))) return res.status(400).json({ ok:false, message:"Payment amount does not match the order total." }); if (order && order.deliveryFee === null) return res.status(400).json({ ok:false, message:"Delivery charge must be confirmed before online payment." }); if (!SHORTCODE) return res.status(500).json({ ok:false, message:"M-PESA shortcode is not configured." }); if (!CALLBACK_BASE_URL.startsWith("https://")) return res.status(500).json({ ok:false, message:"M-PESA callback URL is not configured as HTTPS." }); const token = await getAccessToken(); const timestamp = makeTimestamp(); const payload = { BusinessShortCode:SHORTCODE, Password:stkPassword(timestamp), Timestamp:timestamp, TransactionType:"CustomerPayBillOnline", Amount:amount, PartyA:phone, PartyB:SHORTCODE, PhoneNumber:phone, CallBackURL:`${CALLBACK_BASE_URL}/api/mpesa/callback`, AccountReference:accountReference, TransactionDesc:"FARMTEK09 order" }; const r = await fetch(`${MPESA_BASE_URL}/mpesa/stkpush/v1/processrequest`, { method:"POST", headers:{Authorization:`Bearer ${token}`,"Content-Type":"application/json"}, body:JSON.stringify(payload) }); const data = await r.json().catch(()=>({})); if (!r.ok || data.ResponseCode !== "0") return res.status(502).json({ ok:false, message:data.errorMessage || data.ResponseDescription || "Safaricom did not accept the payment request." }); transactions.set(data.CheckoutRequestID, { status:"pending", phone, amount, accountReference, orderId:order?.id || null, merchantRequestId:data.MerchantRequestID, createdAt:Date.now() }); if (order) { order.paymentMethod="mpesa"; order.paymentStatus="pending"; order.status="awaiting_payment"; order.updatedAt=new Date().toISOString(); order.timeline=Array.isArray(order.timeline)?order.timeline:[]; order.timeline.push({status:"awaiting_payment",at:order.updatedAt}); await writeOrders(orders); } res.json({ ok:true, message:data.CustomerMessage || "STK prompt sent.", checkoutRequestId:data.CheckoutRequestID, orderId:order?.id || null }); } catch(error) { console.error(error); res.status(500).json({ok:false,message:error.message || "Unable to start M-PESA payment."}); } });
app.post("/api/mpesa/callback", async (req,res) => { try { const callback=req.body?.Body?.stkCallback; if(!callback) return res.status(400).json({ResultCode:1,ResultDesc:"Invalid callback payload"}); const tx=transactions.get(callback.CheckoutRequestID); if(tx){ tx.status=Number(callback.ResultCode)===0?"success":"failed"; tx.resultCode=Number(callback.ResultCode); tx.resultDesc=callback.ResultDesc||""; tx.receipt=callback.CallbackMetadata?.Item?.find((item)=>item.Name==="MpesaReceiptNumber")?.Value||null; tx.completedAt=Date.now(); transactions.set(callback.CheckoutRequestID,tx); if(tx.status==="success") await applySuccessfulPayment(tx.orderId,tx); } res.json({ResultCode:0,ResultDesc:"Accepted"}); } catch(error) { console.error("Callback error:",error); res.json({ResultCode:0,ResultDesc:"Accepted"}); } });
