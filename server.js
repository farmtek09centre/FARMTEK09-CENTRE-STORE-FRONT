...
app.get("/api/catalogue",async(_req,res)=>{try{res.json(await readCatalogue())}catch(e){res.status(500).json({message:"Catalogue unavailable."})}});
app.post("/api/admin/login",(req,res)=>{if(!ADMIN_PASSWORD||!crypto.timingSafeEqual(Buffer.from(String(req.body?.password||"")),Buffer.from(ADMIN_PASSWORD)))return res.status(401).json({message:"Invalid admin password."});const token=crypto.randomBytes(32).toString("hex");adminSessions.set(token,Date.now()+8*60*60*1000);res.json({token});});
